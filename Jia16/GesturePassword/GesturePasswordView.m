...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIView * view = [[UIView alloc]initWithFrame:CGRectMake(frame.size.width/2-160, frame.size.height/2-80-68, 320, 320)];
        self.backgroundColor=Color_Macro(242,242,242,1);
        for (int i=0; i<9; i++) {
            NSInteger row = i/3;
            NSInteger col = i%3;
            // Button Frame
        ...
        state = [[UILabel alloc]initWithFrame:CGRectMake(frame.size.width/2-140, frame.size.height/2-120-63, 280, 30)];
        [state setTextAlignment:NSTextAlignmentCenter];
        [state setFont:[UIFont systemFontOfSize:14.f]];
        [self addSubview:state];
        
        
        userName = [[UILabel alloc]initWithFrame:CGRectMake(frame.size.width/2-140,CGRectGetMinY(state.frame)-20, 280, 30)];
        [userName setTextAlignment:NSTextAlignmentCenter];
        [userName setFont:[UIFont systemFontOfSize:16.f]];
        userName.textColor=Color_Macro(53,53,53,1);
...
//        [forgetButton setTitleColor:Color_Macro(53,53,53,1) forState:UIControlStateNormal];
//        [forgetButton setTitle:@"其他账号登录" forState:UIControlStateNormal];
//        [forgetButton addTarget:self action:@selector(change) forControlEvents:UIControlEventTouchDown];
//        [self addSubview:forgetButton];
        ...
        [changeButton setTitleColor:Color_Macro(53,53,53,1) forState:UIControlStateNormal];
        [changeButton setTitle:@"忘记手势密码" forState:UIControlStateNormal];
        [changeButton addTarget:self action:@selector(forget) forControlEvents:UIControlEventTouchDown];
...
- (void)gestureTouchBegin {
    [tentacleView.touchedArray removeAllObjects];
    [tentacleView.touchesArray removeAllObjects];
    [self.state setText:@""];
}
